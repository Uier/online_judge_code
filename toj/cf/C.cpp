...
int main() {
	...
	cin>>t>>a>>b;
	if(a==b){
		bigN x=min(min(a,b),t);
		bigN gcd=get_gcd(x,t);
		cout<<x/gcd<<'/'<<t/gcd<<'\n';
	}else{
		bigN x=min(a,b);
		bigN lcm=a*b/get_gcd(a,b);
		bigN times=(t/lcm+1)*x-1;
		if(lcm>t) times=min(min(a,b)-1,t);
		bigN gcd=get_gcd(times,t);
		cout<<times/gcd<<'/'<<t/gcd<<'\n';
	}
	return 0;
}
